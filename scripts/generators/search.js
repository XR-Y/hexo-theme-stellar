...
function normalizeContent(rawContent, cfg = {}) {
  let content = rawContent || ''

  if (cfg.codeblock === false) {
    content = content.replace(/<figure class="highlight[\s\S]*?<\/figure>/g, ' ')
    content = content.replace(/<pre[\s\S]*?<\/pre>/g, ' ')
    content = content.replace(/<code[\s\S]*?<\/code>/g, ' ')
  }

  content = stripHTML(content.replace(/<span class="line">\d+<\/span>/g, '')).trim()
  ...
  hexo.extend.generator.register('search_json_generator', function (locals) {
    ...
    function generateJson(post) {
      ...
      if (cfg.content != false && post.content) {
        temp_post.content = normalizeContent(post.content, cfg)
      }
